...
( function ( $, config ) {
  ...
  /* ------------------------------------------------------------------ */
  /* Updater Credentials — Save & Delete                                 */
  /* ------------------------------------------------------------------ */
  function updaterNotice( message, type ) {
    const el = document.getElementById( 'sn-updater-notice' );
    if ( ! el ) return;
    el.innerHTML = '<div class="sn-notice sn-notice--' + ( type || 'success' ) + '">' + message + '</div>';
    setTimeout( function () { el.innerHTML = ''; }, 4000 );
  }

  $( document ).on( 'click', '#sn-save-updater-btn', function () {
    const repoUrl = $.trim( $( '#sn_github_repo_url' ).val() );
    const token   = $.trim( $( '#sn_github_token' ).val() );
    const nonce   = $( '#sn-nonce' ).val() || ( window.snAdmin && window.snAdmin.nonce );

    if ( ! repoUrl ) {
      updaterNotice( 'Repository URL is required.', 'error' );
      return;
    }
    ...
    $.ajax( {
      ...
      type: 'POST',
      data: {
        action:   'sn_save_updater_credentials',
        nonce:    nonce,
        repo_url: repoUrl,
        token:    token,
      },
      success: function ( res ) {
        if ( res.success ) {
          updaterNotice( res.data.message, 'success' );
          // Clear the token field — never show the saved value.
          $( '#sn_github_token' ).val( '' );
        } else {
          updaterNotice( ( res.data && res.data.message ) || 'An error occurred.', 'error' );
        }
      },
      error: function () { updaterNotice( 'Request failed.', 'error' ); },
      complete: function () {
        $btn.prop( 'disabled', false ).html(
          '<span class="dashicons dashicons-admin-network"></span> Save Update Credentials'
        );
      },
    } );
  } );

  $( document ).on( 'click', '#sn-delete-updater-btn', function () {
    if ( ! window.confirm( 'Remove GitHub update credentials? The plugin will no longer check for automatic updates.' ) ) return;
    ...
    $.ajax( {
      url:  ajax,
      type: 'POST',
      data: { action: 'sn_delete_updater_credentials', nonce: nonce },
      success: function ( res ) {
        if ( res.success ) {
          updaterNotice( res.data.message, 'success' );
          $( '#sn-delete-updater-btn' ).remove();
          $( '#sn_github_repo_url' ).val( '' );
          $( '#sn_github_token' ).val( '' );
        }
      },
    } );
